      ...
      checks.forEach(function(check){
        ...
        _data.read('checks', check, function(err, originalCheckData){
          if(!err && originalCheckData){
            //Pass it to check validator and let that function continue
...
//Perform check data, send the originalCheckData and the outcome to the next step in the process
workers.performCheck = function(originalCheckData){
  ...
  //Instantiate the request object (either using http or https module)
...
workers.processCheckOutcome = function(originalCheckData, checkOutcome){
  //Decide if the state is considered up or down
  ...
  _data.update('checks', newCheckData.id, newCheckData, function(err){
    ...
    //Send the new check data to the next phase in the process if needed
      if(alertWarranted){
        ...
        debug('Check out come has not changed, no alert needed')
      }
    }else{
...
//Alert to user as there is change in status
workers.alertUserToStatusChange = function(newCheckData){
  var msg = 'Alert : Your check for '+ newCheckData.method.toUpperCase()+ ' '+ newCheckData.protocol+ '://'+ newCheckData.url+' is currently ' + newCheckData.state;
  helpers.sendTwilioSms(newCheckData.userPhone, msg, function(err){
    if(!err){
      debug("Success : User has been alerted to a status change in their check via SMS", msg);
    }else{
      debug("Error Could not send sms alert to user who had a state change in check")
    }
  }) 
};
...
workers.log = function(originalCheckData, checkOutcome,state, alertWarranted, timeOfCheck){
  ...
  _logs.append(logFileName, logString, function(err){
    if(!err){
      debug('Logging the file succeeded');
    }else{
